...
formationsRoutes.get("/getFormation/:formationId", async (req, res) => {
    const { formationId } = req.params;
    try {
        const requete = `SELECT * FROM formations WHERE idFormation = ?`;
        const [resultats] = await baseDeDonnees.query(requete, [formationId]);
...
formationsRoutes.put("/decrement_place/:idFormation", async (req, res) => {
    const { idFormation } = req.params
    try {
        const requete = `UPDATE formations
                         SET capacite = capacite - 1
                         WHERE idFormation = ? and capacite > 0`;
